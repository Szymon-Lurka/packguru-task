<template>
  <div class="chunk-panel">
    <div class="panel-header">
      ...
      <button class="close-btn" :title="t('chunk.closeTitle')" @click="emit('close')">&#x2715;</button>
    </div>
        ...
        <ul class="source-list">
          <li v-for="(s, i) in chunk.sources" :key="i" class="source-item">
            ...
            <span class="source-meta">
              {{ t('chunk.partWord') }} {{ s.part_index }}{{ fmtTimeRange(s.start_seconds, s.end_seconds) }}
            </span>
...
const typeLabel = computed(() => {
  const key = `chunk.types.${props.chunk.type}`
  return te(key) ? t(key) : props.chunk.type
})
